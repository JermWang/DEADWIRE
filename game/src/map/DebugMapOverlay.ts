...
function makeLine(start: Vec3, end: Vec3, color: number): THREE.Line {
...
function makeWireBox(size: Vec3, position: Vec3, color: number): THREE.LineSegments {
...
function makeLabel(text: string, position: Vec3, color = '#ffffff'): THREE.Sprite | null {
...
export class DebugMapOverlay {
  ...
  private addPOIDebug(root: THREE.Group, poi: POIDefinition): void {
    ...
    if (this.options.showLabels) {
      const label = makeLabel(
        `${poi.name} · D${poi.dangerLevel}`,
        { x: poi.position.x, y: topY, z: poi.position.z },
        `#${dangerColor.toString(16).padStart(6, '0')}`,
      );
      if (label) root.add(label);
    }

    if (!this.options.showVolumes) return;
    ...
    for (const floor of poi.floors) {
      root.add(makeWireBox(
        { x: floor.size.x, y: 0.12, z: floor.size.z },
        { x: poi.position.x, y: floor.y + 0.08, z: poi.position.z },
        floor.y < 0 ? 0x4b62ff : 0xffffff,
      ));
    ...
    for (const loot of poi.lootZones) this.addLootDebug(root, loot);
    for (const cover of poi.coverZones) this.addCoverDebug(root, cover);
  ...
  private addLootDebug(root: THREE.Group, loot: LootZoneDefinition): void {
    ...
    const label = makeLabel(
      `${loot.lootTier}${loot.isLocked ? ' LOCKED' : ''}`,
      { x: loot.position.x, y: loot.position.y + loot.size.y / 2 + 0.8, z: loot.position.z },
      loot.isLocked ? '#ff7ae6' : '#8dffbc',
    );
    if (label) {
  ...
  private addCoverDebug(root: THREE.Group, cover: CoverZoneDefinition): void {
    root.add(makeWireBox(
      { x: cover.size.x, y: cover.height, z: cover.size.z },
      { x: cover.position.x, y: cover.position.y + cover.height / 2, z: cover.position.z },
      0x75a7ff,
    ));
  }

  private addTraversalDebug(root: THREE.Group, link: TraversalLinkDefinition): void {
