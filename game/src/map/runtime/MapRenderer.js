...
export class MapRenderer {
    ...
    setDebug(root, map, enabled) {
        const existing = root.getObjectByName(`${map.id}_debug_overlay`);
        if (existing)
            root.remove(existing);
        if (enabled)
            root.add(new DebugMapOverlay().render(map));
    ...
    createGround(map) {
        ...
        const ground = new THREE.Mesh(new THREE.BoxGeometry(width, 0.08, depth), this.material('ground', 0x303936));
        ground.position.set((map.bounds.min.x + map.bounds.max.x) / 2, -0.06, (map.bounds.min.z + map.bounds.max.z) / 2);
    ...
    createPOI(poi) {
        ...
        for (const floor of poi.floors) {
            const platform = new THREE.Mesh(new THREE.BoxGeometry(floor.size.x, this.options.floorThickness, floor.size.z), this.material(floor.y < 0 ? 'floor_underground' : 'floor', floor.y < 0 ? 0x1b2331 : 0x465150));
        ...
        for (const entrance of poi.entrances) {
            const marker = new THREE.Mesh(new THREE.BoxGeometry(entrance.width, 0.16, 0.32), this.material(entrance.isLocked ? 'locked_entrance' : 'entrance', entrance.isLocked ? 0xe044b6 : 0x60ff9a));
            marker.name = entrance.id;
            marker.position.set(entrance.position.x - poi.position.x, entrance.position.y - poi.position.y + 0.12, entrance.position.z - poi.position.z);
            group.add(marker);
        }
    ...
    createBuildingMass(poi) {
        const color = COLORS[poi.category] ?? 0x89949b;
        const mass = new THREE.Mesh(new THREE.BoxGeometry(poi.size.x, poi.size.y, poi.size.z), this.material(`${poi.category}_mass`, color, 0.32));
    ...
    createVerticalConnector(connector, origin) {
        if (connector.type === 'stairs')
            return this.createStairs(connector, origin);
        if (connector.type === 'ramp')
            return this.createRamp(connector, origin);
        if (connector.type === 'ladder')
            return this.createLadder(connector, origin);
        return this.createElevator(connector, origin);
    }
    createStairs(connector, origin) {
        const group = new THREE.Group();
        ...
        for (let i = 0; i < steps; i++) {
            ...
            const step = new THREE.Mesh(new THREE.BoxGeometry(connector.width ?? 1.4, stepHeight, stepDepth), this.material('stairs', 0xb7aa8d));
            step.position.set(pos.x - origin.x, connector.start.y - origin.y + Math.sign(rise || 1) * stepHeight / 2, pos.z - origin.z);
            step.rotation.y = yaw;
    ...
    createRamp(connector, origin) {
        const mid = this.lerp(connector.start, connector.end, 0.5);
        const dx = connector.end.x - connector.start.x;
        const dz = connector.end.z - connector.start.z;
        const length = Math.hypot(dx, dz);
        const ramp = new THREE.Mesh(new THREE.BoxGeometry(connector.width ?? 2, 0.18, length || 1), this.material('ramp', 0x9ba68f));
    ...
    createElevator(connector, origin) {
        const group = new THREE.Group();
        group.name = connector.id;
        const height = Math.abs(connector.end.y - connector.start.y);
        const mid = this.lerp(connector.start, connector.end, 0.5);
        const shaft = new THREE.Mesh(new THREE.BoxGeometry(1.8, height, 1.8), this.material('elevator_shaft', 0x6bd6ff, 0.18));
        shaft.position.set(mid.x - origin.x, mid.y - origin.y, mid.z - origin.z);
        group.add(shaft);
        group.userData.traversal = connector;
        return group;
    }
    createLootZone(loot, origin) {
        const group = new THREE.Group();
        group.name = loot.id;
        ...
        for (let i = 0; i < count; i++) {
            ...
            crate.position.set(loot.position.x - origin.x - loot.size.x / 3 + col * 0.8, loot.position.y - origin.y + 0.28, loot.position.z - origin.z - loot.size.z / 4 + row * 0.8);
            group.add(crate);
        }
        return group;
    }
    createCoverZone(cover, origin) {
        const coverMesh = new THREE.Mesh(new THREE.BoxGeometry(cover.size.x, cover.height, cover.size.z), this.material(`cover_${cover.coverType}`, 0x6f7880));
        coverMesh.name = cover.id;
        coverMesh.position.set(cover.position.x - origin.x, cover.position.y - origin.y + cover.height / 2, cover.position.z - origin.z);
        return coverMesh;
    }
    createTraversalLink(link) {
        ...
        const color = link.type === 'underground_tunnel'
            ...
            : link.type === 'rooftop_bridge'
                ? 0xb89042
                : 0x52645b;
        const surface = new THREE.Mesh(new THREE.BoxGeometry(this.linkWidth(link), 0.18, length), this.material(`link_${link.type}`, color));
    ...
    linkWidth(link) {
        if (link.type === 'rooftop_bridge')
            return 2.2;
        if (link.type === 'underground_tunnel')
            return 3;
        if (link.type === 'alley_shortcut')
            return 2.4;
        return 2.8;
    }
    material(name, color, opacity = 1) {
