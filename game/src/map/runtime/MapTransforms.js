function translatedPoint(point, oldPOI, newPosition) {
    return {
 ...
 */
export function expandMapLayout(source, horizontalScale) {
    const center = {
    ...
    const pois = source.pois.map((poi) => {
        ...
        const move = (point) => translatedPoint(point, poi, position);
        const moveConnector = (connector) => ({
            ...connector,
    ...
    const traversalLinks = source.traversalLinks.map((link) => {
...
//# sourceMappingURL=MapTransforms.js.map